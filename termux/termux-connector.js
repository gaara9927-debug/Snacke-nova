...
tiktokConnection.on(WebcastEvent.GIFT, async (data) => {
  try {
    ...
    const payload = {
      event: 'gift',
      eventId,
      username: data.uniqueId || data.user?.uniqueId || data.user?.unique_id || data.user?.nickname || 'tiktok_user',
      nickname: data.nickname || data.user?.nickname || data.user?.uniqueId || data.user?.unique_id || 'TikTok User',
      userId: String(data.userId || data.user?.userId || data.user?.user_id || ''),
      profilePicture: data.profilePictureUrl || data.user?.profilePictureUrl || data.user?.profile_picture?.url_list?.[0] || '',
